...
- **高速道路**（OSM Overpass API）: IC/JCT は `motorway_junction` ノード、SA/PA は `services|rest_area`。親 way（道路名・operator）は全国一括クエリだと 504/応答途切れが頻発するため、junction ノード ID を 500 件ずつのバッチで問い合わせて `ways-part-*.json` に分割キャッシュし `ways.json` にマージする。エンドポイントは overpass-api.de → kumi.systems → osm.jp をローテーション（429/504 で次へ。osm.jp は証明書切れのことがある）。空 elements や `remark` 付き応答はキャッシュに書かない（エラーがデータに見える事故防止）。名前の正規化はコード側（`lib/highway.ts`: 方向サフィックス除去・入口/出口→出入口・料金所/BS 除外）で行い、すり抜けは `overrides/highway-fixes.json`（`exclude`/`rename`、キーは OSM id `n123…`）。本線線形（オーバーレイ用 `lines.json`）は `line-ids.json` で ID 一覧を取ってから ID バッチで `out geom` 取得（`lines-part-*.json` に分割キャッシュ、全国一括 `out geom` は数十MBで途切れるので不可）。キャッシュ無効化は `data-cache/highway/` を削除（線形も一括で再取得される）。build は pref 判定に `data-cache/geo/muni/` にも依存する（`npm run data:highways` が正しい順で実行する）。
- **国道**（OSM Overpass API）: `route=road` リレーション全件のタグを取り、`network=JP:national` または name「国道N号」で国道を選別（旧道・旧国道は除外、バイパスは含める）。数値 ref だけでは県道と区別できない点に注意。メンバー way の線形取得は `["highway"]` フィルタ付き ID バッチで行い、フェリー・渡船の海上 way を機械的に除外する（42号・58号等）。スーパーリレーションは追加パスで展開。build（`build-route-lines.ts`）は 1〜507 の欠番ルール（59〜100・109〜111・214〜216）から期待集合459路線を生成して突合し、欠けたら exit 1 → `overrides/route-fixes.json` の `addRelations`（誤選別は `exclude`、キーは `r<リレーションID>`）に追記して再実行。**overrides を編集したら `data-cache/routes/` を削除してから再実行する**（メンバー・線形キャッシュが選別結果に依存するため）。pref 判定は `data-cache/geo/muni/` に依存（`npm run data:routes` が正しい順で実行する）。
